...
typedef void fn_mlmc_sample_levels(unsigned int L,
                                   const unsigned long long* M,
                                   double *sums,
                                   unsigned int sums_size,
                                   void *user_data);

typedef void fn_mlmc_sample_level(unsigned int ell,
                                  unsigned long long M,
                                  double *sums,
                                  unsigned int sums_size,
                                  void *user_data);

mlmc_options* MLMCCreateOptions();
void MLMCFreeOptions(mlmc_options*);

mlmc_output* MLMCRun(double eps, fn_mlmc_sample_levels sampler,
                     const mlmc_options*); // Options can be NULL for default
void MLMCFreeOutput(mlmc_options*);

// This function call a simple single-level sampler
// for every level.
void MLMCSampleLevels(fn_mlmc_sample_level lvl_sampler,
                      unsigned int L,
                      const unsigned long long* M,
                      double *sums,
                      unsigned int sums_size,
...
void MLMCEstimateError(const mlmc_output* error, double* discretization_error,
                       double *stat_error);
void MLMCComputeEstimate(const mlmc_output* error,
                         double *estimate,
